...
int main(int argc, char* argv[]) {
        ...
        while (getline(infile, str)){ //Read a line from the file
            ...
            if (inGlobalMain == 1 && str.find(".") == notThere) { // this is where we handle instructions and instruction labels
                ...
                if (str.find(": .") != notThere) { 
                    ...
                    std::string __STATICMEMORYEND__ ;
                    staticVector.push_back(__STATICMEMORYEND__);
                }
            }
        }
        infile.close();
    ...
    int numVal = 0;
    int staticCounter = 0;
    // staticCounyter= staticVector.size()*4;
    // write_binary(staticCounter, static_outfile);
    for(std::string value1 : staticVector) {
        //staticCounter = staticCounter + 4       //keep track of bits useed up by static memory
    ...
    //std::string inst = "addi $sp, $0, 65280";
    //int sep = inst.find(" ");
